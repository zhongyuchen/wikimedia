...
public class LosslessID {
    ...
    public static class LosslessIDMapper extends Mapper<LongWritable, Text, WordID, Text>{
...
*/
            ...
            for (String w: wordPos.keySet()) {
                ...
                for (Long position: wordPos.get(w)) {
                    ...
                    posList.append(posDiff.toString());
                    posList.append(',');
                    posPast = position;
                }
                posList.insert(0, '-');
    ...
    public static class LosslessIDReducer extends Reducer<WordID, Text, Text, Text> {
        ...
        public void reduce(WordID key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
            ...
            for(Text value : values) {
                ...
                length = length + 1L;
                idDiff = id - idPast;
                //fileList += idDiff.toString() + "-" + split[1] + ";";
                fileList.append(idDiff);
                fileList.append('-');
                fileList.append(split[1]);
                fileList.append(';');
                idPast = id;
            }
            if (length > 0) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf, LosslessID.class.getName());
        job.setJarByClass(LosslessID.class);
        job.setNumReduceTasks(15);

        job.setInputFormatClass(XmlInputFormat.class);
        job.setOutputFormatClass(TextOutputFormat.class);

        job.setMapperClass(LosslessIDMapper.class);
        job.setMapOutputKeyClass(WordID.class);
        job.setMapOutputValueClass(Text.class);

        job.setGroupingComparatorClass(WordIDGroup.class);
        job.setReducerClass(LosslessIDReducer.class);
